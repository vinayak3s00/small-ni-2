...
def test_emit_stamps_event_shape():
    ...
    m = MeterEmitter(service="concierge-agent", sink=sink)
    m.count("ai_actions", "tenant-a", event_id="e1", source="inbound:price_quote")
    ...
    assert e["quantity"] == 1
    assert e["eventId"] == "e1"
    assert e["service"] == "concierge-agent"
    assert e["source"] == "inbound:price_quote"
    assert "at" in e
