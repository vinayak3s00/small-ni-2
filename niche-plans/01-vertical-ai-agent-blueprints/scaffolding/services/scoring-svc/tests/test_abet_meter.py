...
def test_emit_stamps_event_shape():
    ...
    m = MeterEmitter(service="scoring-svc", sink=sink)
    m.count("ai_actions", "tenant-a", event_id="e1", source="score:realty")
    ...
    assert e["quantity"] == 1
    assert e["eventId"] == "e1"
    assert e["service"] == "scoring-svc"
    assert e["source"] == "score:realty"
    assert "at" in e
