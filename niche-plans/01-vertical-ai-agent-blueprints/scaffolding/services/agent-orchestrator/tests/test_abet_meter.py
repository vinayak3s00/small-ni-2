...
def test_emit_stamps_event_shape():
    ...
    m = MeterEmitter(service="agent-orchestrator", sink=sink)
    m.count("ai_actions", "tenant-a", event_id="e1", source="answer:realty")
    ...
    assert e["quantity"] == 1
    assert e["eventId"] == "e1"
    assert e["service"] == "agent-orchestrator"
    assert e["source"] == "answer:realty"
    assert "at" in e
