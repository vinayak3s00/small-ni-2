 ...
 */
export async function resolveStore(): Promise<{ store: OrderStore; dbPing?: ReadinessCheck }> {
  if (!process.env.DATABASE_URL) return { store: new InMemoryOrderStore() };
  ...
  return { store: new PgOrderStore(pool), dbPing };
}
